...
class TestModelUnitsAreNotDegenerate:
    ...
    def test_returns_are_in_the_callers_units(self):
        ...
        assert np.all(lo <= y_cal) and np.all(y_cal <= hi)
...
class TestEdgeCases:
    def test_constant_observations_fall_back_to_unit_scale(self):
        sim = np.linspace(1e-6, 1e-4, 20)
        obs = np.full(20, 2e-5)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DegenerateCalibrationWarning)
